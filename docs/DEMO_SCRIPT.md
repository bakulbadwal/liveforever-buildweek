...
Target length: 2 minutes 40 seconds. Record at 1280 x 720 or higher with clear voice audio. Do not use copyrighted music.
...
"This is LiveForever, a privacy-first personal evidence lab built for OpenAI Build Week. It turns wearable history, habits, labs, and cautious genomic context into one testable wellness question, with uncertainty shown instead of hidden."
...
## 2:13-2:34 — Codex And GPT-5.6

**On screen:** Show `SKILL.md`, `analysis.py`, and `BUILD_WEEK_PROVENANCE.md`. Briefly show this Codex build task.

**Say:**

"Codex was my primary build environment. It designed and implemented the analysis contract, tests, synthetic data, safety boundaries, interface, and submission package. GPT-5.6 is also the reasoning layer in the installed Skill: it frames the question, verifies primary sources, explains the fixed calculations, finds blind spots, and adapts the experiment without changing the numbers or making medical claims."
...
- Show the working interface, not slides alone.
- Audio must explain the Codex contribution and GPT-5.6 product role.
- Upload to YouTube as **Public**, not Unlisted or Private.
- Replay the uploaded video and check the audio before pasting it into Devpost.
